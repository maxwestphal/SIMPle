...
check_moments <- function(moments, nu, input, project, msg=TRUE, ...){
  ...
  if(project){
    if(any(B$below)){
      if(msg) message("SIMPle: Some ", input.str, " are too low - projecting onto Frechet bounds!")
    }
    if(any(B$above)){
      if(msg) message("SIMPle: Some ", input.str, " are too high - projecting onto Frechet bounds!")
    }
    if(!all(B$inside)){
      if(msg) message("SIMPle: ", input.str, " conforming to Frechet bounds:")
  ...
  if(!project){
    if(!all(B$inside)){
      if(msg) message("SIMPle: ", input.str, " conforming to Frechet bounds:")
      if(msg) print(B$inside)
      stop("Frechet bounds (partially) violated!")
    }
    return(B$projected)
  }
}
